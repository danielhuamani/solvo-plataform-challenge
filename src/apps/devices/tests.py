...
class DeviceFlowsTests(TestCase):
    def setUp(self):
        ...
        self.platform = Platform.objects.create(
            name="Plataforma A", slug="plataforma-a", is_active=True
        )
        PlatformSetting.objects.create(
            platform=self.platform, max_devices=2, allow_inactive_login=False
        )
    ...
    def test_devices_list_returns_only_authenticated_users_devices(self):
        Device.objects.create(
            platform_user=self.user1, name="d1", ip_address="127.0.0.1", is_active=True
        )
        Device.objects.create(
            platform_user=self.user2, name="d2", ip_address="127.0.0.2", is_active=True
        )
