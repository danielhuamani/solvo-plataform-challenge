...
class AuthFlowsTests(TestCase):
    def setUp(self):
        ...
        self.platform_a = Platform.objects.create(
        ...
        self.platform_b = Platform.objects.create(
            name="Plataforma B", slug="plataforma-b", is_active=True
        )
        PlatformSetting.objects.create(
    ...
    def test_register_success_triggers_notification(self, logger_info):
        ...
        self.assertTrue(
            PlatformUser.objects.filter(
                platform=self.platform_a, email="user@email.com"
            ).exists()
        )
